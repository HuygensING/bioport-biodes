...
#        'namen_en',
    ...
    _key_mapping_en = {}
    for key in _keys:
        _key_mapping_nl[key[0]] = dict(zip(('path', 'type'), key[1:]))
        _key_mapping_en[_translate(key[0])] = dict(zip(('path', 'type'), key[1:]))
...
#            print args[k]
    ...
    #INFO OVER PERSOON
        ...
        k = 'beroep'
        ls = args.get(k, args.get(_translate(k)))
        if type(ls) in types.StringTypes:
            ls = [ls]
        if ls:
            for s in ls:
...
#        assert len(ls) <= 1, ls
    ...
    def to_file(self, fn):
        open(fn, 'w').write(self.to_string())
